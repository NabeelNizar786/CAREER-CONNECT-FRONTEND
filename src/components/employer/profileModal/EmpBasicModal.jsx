...
export default function EmpBasicModal({empData}) {
  ...
  const handleSubmit = (e) => {
    ...
    if (info.name.trim() == "") {
      return toast.error("name is needed");
    }
    if (info.Location.trim() == "") {
      return toast.error("location is needed");
    }
    if (!/^(\+\d{1,3})?\d{10}$/.test(info.Phone)) {
      return toast.error("Mobile number should be a valid 10-digit number");
    }
