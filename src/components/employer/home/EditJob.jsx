...
export default function EditJob({ post, skills, citys, setPosts }) {
  ...
  const handleCheckboxChange = selectedOptions => {
    // Update the state to reflect the new selected skills
    setIsChecked(selectedOptions.map(option => option.value));
    console.log(selectedOptions.map(option => option.value)); // Use the updated selectedOptions directly
  };
  ...
  const handleSubmit = async (e) => {
    ...
    jobData.skills = selectedSkills.map(skill => skill.toString().trim());
    ...
    <>
      <div className="flex justify-end ">
        <button
          ...
          className="bg-green-800 ms-3 text-white text-sm md:text-lg px-2 md:p-1 p-1 md:px-3 font-semibold rounded-md hover:bg-green-400"
        >
          EDIT
        </button>
        {showModal ? (
