...
export default function ConfirmBlockEmp ({emp, change, onClose}) {

  const dispatch = useDispatch();

  const handleBlock = () => {
    dispatch(showLoading());
    const newStatus = emp.status ? "false" : "true";
    
    change(emp._id, newStatus);
    
    setTimeout(() => {
      dispatch(hideLoading());
      onClose(); // Close the confirmation modal
      const action = newStatus === "true" ? "Unblocked" : "Blocked";
      toast.success(`Employer ${action} successfully`);
    ...
    <div className="fixed inset-0 flex justify-center items-center z-10">
      <div className="bg-white dark:bg-neutral-800 p-6 rounded shadow-md w-96 h-auto">
        <p className="text-lg font-bold">
          ...
          {emp.status ? "block" : "unblock"} this user?
        </p>
